...
class _AvailableRidesPageState extends State<AvailableRidesPage> {
  ...
  @override
  void initState() {
    context.read<EarningsSummaryBloc>().add(const EarningsSummaryEvent.getEarningsSummary());
    context.read<AvailableRideBloc>().add(const AvailableRideEvent.requestData());
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        ...
        child: BlocListener<UpcomingRideBloc, UpcomingRideState>(
          listener: (context, state) {
            if (state.status == UpcomingRideStatus.actionLoading) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(state.errorResponse?.message ?? "Failed to accept ride."),
          ...
          child: RefreshIndicator(
            onRefresh: () async {
              context.read<AvailableRideBloc>().add(const AvailableRideEvent.refreshData());
            },
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(state.errorResponse?.message ??
                                  'Something went wrong', style: const TextStyle(fontSize: 16, color: Colors.black87)),
                              const SizedBox(height: 16),
                              ElevatedButton(
                                onPressed: () {
                                  context.read<AvailableRideBloc>().add(
                                      const AvailableRideEvent.refreshData());
                                },
                        ...
                        child: Center(
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 16),
                              Text('No rides available right now.', style: TextStyle(fontSize: 16, color: Colors.grey.shade600)),
                            ],
                          ),
                        ),
                      );
                            ...
                            child: RideCard(
                              ...
                              onAccept: () {
                                context
                                    ...
                                    .add(UpcomingRideEvent.accept(id: ride.id));
                              },
                            ),
                          );
                        },
