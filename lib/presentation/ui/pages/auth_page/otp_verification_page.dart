  ...
  void _verifyOtp(String pin) {
    context.read<OtpVerificationBloc>().add(
          OtpVerificationEvent.verify(
            email: widget.email,
            otp: pin,
            isForgotPassword: widget.verificationType == VerificationType.forgotPassword,
          ),
        );
  }
          ...
          listener: (context, state) {
            if (state.status == OtpVerificationStatus.success) {
              ...
              if (otpError == "otpResendCooldown") {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(state.errorResponse?.message ?? 'Failed to resend OTP'),
                    backgroundColor: Colors.red,
                  ),
                );
              }
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        TextButton(
                          onPressed: (state.status == OtpVerificationStatus.resendLoading || _cooldownSeconds > 0)
                              ? null
                              : _resendOtp,
                          child: state.status == OtpVerificationStatus.resendLoading
                              ? const SizedBox(
                                  width: 16,
                                  height: 16,
                              ...
                              : Text(
                                  ...
                                  style: TextStyle(
                                    fontWeight: FontWeight.w700,
                                    color: _cooldownSeconds > 0 ? Colors.grey : primaryColor,
                                  ),
                                ),
                        ),
                      ],
