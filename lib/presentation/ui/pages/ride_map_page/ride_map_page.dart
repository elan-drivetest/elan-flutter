...
import 'package:elan/presentation/ui/pages/ride_map_page/bottom_sheet/pickup_bottom_sheet.dart';
...
class RideMapPage extends StatefulWidget {
  const RideMapPage({super.key, required this.rideInfo});
  final Ride rideInfo;
...
class _RideMapPageState extends State<RideMapPage> {
  ...
  void _openBottomSheet() {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => PickupBottomSheet(rideInfo: widget.rideInfo),
    );
  }
  ...
  void initState() {
    ...
    // Inferring it from a null pickup routes the instructor to a stale pickup
    // address the moment the two disagree (RIDE_JOURNEY §2).
    ...
    if (ride.isMeetAtCentre) {
      ...
      // A pickup ride whose coordinates never arrived. Show the centre rather
      // than a placeholder, and say so.
      _isShowingTestCenter = true;
      _initialPosition = (testCenterLat != null && testCenterLng != null)
          ? LatLng(testCenterLat, testCenterLng)
          : _fallbackPosition;
